...
function getProviderOrder(preferred, keys) {
    ...
    const withKeys = defaultOrder.filter((p) => (p === "openai" && keys.openaiKey) || (p === "gemini" && keys.geminiKey) || (p === "openrouter" && keys.openRouterKey));
...
async function tryGemini(systemPrompt, contents, jsonMode, geminiKey, geminiModel) {
    ...
    const inputTokens = um?.promptTokenCount != null ? Number(um.promptTokenCount) : um?.prompt_token_count != null ? Number(um.prompt_token_count) : null;
    const outputTokens = um?.candidatesTokenCount != null ? Number(um.candidatesTokenCount) : um?.candidates_token_count != null ? Number(um.candidates_token_count) : null;
    return {
        text,
        provider: "gemini",
        response: resp,
...
export async function callLLM({
    ...
    if (!order.length) {
        return {
            ...
            data: {
                error: {
                    message: "No API key configured. Set OPENAI_API_KEY, GEMINI_API_KEY or OPENROUTER_API_KEY.",
                },
            },
        };
    }
    ...
    for (const provider of order) {
        try {
            if (provider === "openai") {
                lastResult = await tryOpenAI(systemPrompt, contents, jsonMode, openaiKey, openaiModel);
            } else if (provider === "gemini") {
                lastResult = await tryGemini(systemPrompt, contents, jsonMode, geminiKey, geminiModel);
            } else {
                lastResult = await tryOpenRouter(systemPrompt, contents, jsonMode, openRouterKey, openRouterModel);
            }
